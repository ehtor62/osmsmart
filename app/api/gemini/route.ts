...
export async function POST(req: NextRequest) {
  ...
  try {
    const geminiPrompt = `${prompt} ${JSON.stringify(relevantData)}. Only for elements that are truly relevant for a tourist, provide a markdown table with the following columns: name of the element, description, best for, insider tips, latitude (mandatory), longitude (mandatory). Latitude and longitude must always be listed for each element. After the table, write a short summary for tourists. If no elements are relevant, say so and do not provide a table.`;
